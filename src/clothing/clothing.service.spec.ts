...
describe('ClothingService', () => {
  ...
  describe('getById', () => {
    ...
    it('should throw an error if the clothing item does not exist', async () => {
      await expect(clothingService.getById(10000)).rejects.toThrow(
        HttpException,
      );
      await expect(clothingService.getById(10000)).rejects.toThrowError(
        'not found clothing with such id',
      );
    });
  });
  ...
  describe('findBySize', () => {
    it('should get all clothing items with a specific size', async () => {
      ...
      clothingService.findBySize = jest.fn(() =>
        Promise.resolve([mockClothing]),
      );
    ...
    it('should throw an error if no clothing items with the specified size exist', async () => {
      await expect(clothingService.findBySize('XXXXL')).rejects.toThrow(
        HttpException,
      );
      await expect(clothingService.findBySize('XXXXL')).rejects.toThrowError(
        'not found clothing such size',
      );
    });
  });
  ...
  describe('create', () => {
    ...
    it('should throw an error if the clothing item is missing a required field', async () => {
      ...
      await expect(clothingService.create(dto, image)).rejects.toThrowError(
        HttpException,
      );
      await expect(clothingService.create(dto, image)).rejects.toThrowError(
        'input cant be empty',
